...
export function GameScreen({
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-2xl mx-auto px-4 py-6">
      {/* Header */}
      <div className="text-center mb-6">
        <div className="text-sm font-semibold text-gray-500 uppercase tracking-wide">
          Day {puzzle.dayNumber} • {formatDayOfWeek(puzzle.dayOfWeek)} • {formatDifficulty(puzzle.difficulty)}
        </div>
        <div className="mt-1 h-1 w-24 mx-auto bg-gradient-to-r from-blue-400 to-purple-500 rounded-full" />
      </div>
      ...
      {/* Warning Text */}
      <div className="text-center text-sm text-amber-600 font-medium mb-4">
        ⚠️ Choose carefully - you only get one guess!
      </div>
      ...
      {selectedIndex !== null && (
        <button
          ...
          disabled={disabled}
          className="w-full py-4 px-6 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-bold rounded-xl transition-colors duration-200 shadow-lg"
        >
          Lock In Your Answer
        </button>
      )}
